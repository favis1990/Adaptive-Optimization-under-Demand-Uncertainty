## this code is used for sensitivity analysis and simulation
### simulation without folding horizon
### this simulatin also consider folding horizon
...
for coef in [1.2]:
    ...
    jj = 6
    rr = 8
    tt = 11 ###5 day each day one shif plus last day one shift
    iti = 100
    CAP = np.zeros((jj))
    aa= 1
    fixed_cap=1
    ...
    for t in (1,3,5,7,9): #night shift difference 
             q_p[0][t] = fixed_cap*180
             q_p[1][t] = fixed_cap*130
             q_p[2][t] = fixed_cap*130
             q_p[3][t] = fixed_cap*130
    ...
    objdeter ,Z_P_V_deter, Ydeter, Xdeter, Bdeter, Ideter, setup_det_first, setup_det, DETmodeling, DETtotal, backorder_det, prod_det,holding_det, transport_det, opening_det, number_of_DV_RC_model = RC(jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, dz, o_w ,q ,sigma, bigM, C_O_W, ini_inv)
    zaman=open('random_data_BS_october.txt','a')
    zaman.write(str('************')+'     '+str(total_demand)+'     '+str(coef)+'     '+str(coef)+'     '+str(coef)+'     '+str(coef)+'     '+str(coef)+'     '+str(zeta)+'     '+str('************'))
    zaman.write('\n')
    ...
    objRC, Z_P_V_RC, YRC, XRC, BRC, IRC, setup_RC_first, setup_RC, RCmodeling, RCtotal, backorder_RC, prod_RC, holding_RC, transport_RC, opening_RC, number_of_DV_RC_model = RC(jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, D_W, o_w, q, sigma, bigM, C_O_W, ini_inv) 
    zaman=open('random_data_BS_october.txt','a')
    ...
    ARCobj, IARC, alphaARC, ARCprodcost, XARC, betaARC, ARCtranscost, BARC, gammaARC, ARCbackcost, ARCholdcost, ARCmodeling, ARCtotal= ARC(jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, D_W, dz, dh, o_w, q, sigma, bigM, setup_RC, setup_RC_first, opening_RC, Z_P_V_RC, ini_inv) 
    zaman=open('random_data_BS_october.txt','a')
    ...
    (RE_obj, YB, alphaN, RE_prodcost, XB, betaN, RE_transcost, BB, gammaN, RE_backcost, RE_holdcost, RE_modeling, RE_total) = REARC (jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, D_W, dz, dh, o_w, q, sigma, bigM, setup_RC, setup_RC_first, opening_RC, Z_P_V_RC, ARCobj, ini_inv)                               
    zaman=open('random_data_BS_october.txt','a')
    ...
    zaman.close()
    #%% simulation starts
    writefile=open('SIMiulation_case_October.txt','a')
    writefile.write(str('***************'))
    writefile.write(str(jj)+'     '+str(kk)+'     '+str(tt)+'     '+str(rr)+'     '+str(ww)+'     '+str(zeta))
    writefile.write(str('***************'))
    dh=np.zeros((rr,jj,tt))
    ...
    for iteration in range(iti):
        ...
        for period in (list(range(0, tt))):
            for r in range (rr):
                for j in range (jj):
                        nominal_folding_demand[r,j, period] = copy.deepcopy(realized_demand[r,j, period])
            objdeter_fold_nominal, American_value_nominal, disposal_valu_nominal, Z_P_V_fold, Yfold, Xfold, Bfold, Ifold, costdispose_nom, costamrican_nom, setup_det_first_fold, setup_fold,foldmodeling, foldtotal,backorder_fold, prod_fold, holding_fold, transport_fold, opening_fold,number_of_DV_RC_model = RC_fold(carpi, jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, nominal_folding_demand, o_w, q, sigma, bigM, C_O_W, ini_inv, fold_X, fold_Y, fold_Z, fold_I, period,dis_per_unit)
        ...
        for period in (list(range(0, tt))):
            for r in range (rr):
                for j in range (jj):
                        nominal_folding_demand[r,j, period] = copy.deepcopy(realized_demand[r,j, period])
            objdeter_fold_rob, American_value_rob, disposal_valu_rob, Z_P_V_fold_r, Yfold_r, Xfold_r, Bfold, Ifold_r, costdispose_rob, costamrican_rob, setup_det_first_fold, setup_fold, foldmodeling, foldtotal, backorder_fold, prod_fold,holding_fold, transport_fold, opening_fold, number_of_DV_RC_model = RC_fold(carpi, jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, robust_folding_demand, o_w, q, sigma, bigM, C_O_W, ini_inv, fold_X, fold_Y, fold_Z, fold_I,period, dis_per_unit)
    ...
    writefile.write('\n')
    writefile.write(str('**********END OF THIS SET***********'))
    writefile.write('\n')
    writefile.write(str('#############################################################################'))
    writefile.write('\n')
    writefile.write('\n')
    ...
    writefile=open('SIMiulation_case_October.txt','a')
    writefile.write(str('***************'))
    writefile.write(str("back")+'     '+str("back")+'     '+str("back")+'     '+str("back")+'     '+str("back")+'     '+str("back"))
    writefile.write(str('***************'))
    dh=np.zeros((rr,jj,tt))
    realized_demand=np.zeros((rr,jj,tt))
    worsecase_demand=sum(sum(sum((D_W[rp][jp][tz]) for rp in range (rr)) for jp in range (jj)) for tz in range (tt))
