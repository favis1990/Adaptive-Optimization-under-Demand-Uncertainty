...
def REARC2(jj, kk, tt, rr, ww, q_p, q_w, c_b, h, p, transperunit, CAP, D_W, dz, dh, o_w, q, sigma, bigM, C_O_W, ARCobj, ini_inv) :         
    RARCstart = time.time()
    ''' Model '''
    ...
    # Objective Function
    mdl.minimize(setup+ openingw+ setupfirst+ OTHR)
    ...
    mdl.minimize(setup+openingw+setupfirst+mdl.sum(p[j]*((mdl.sum(mdl.sum(I_bar[(w,j,k,0)]-ini_inv[w][j][k]+mdl.sum(mdl.sum(mdl.sum(alpha[(w,j,k,0,rp,jp,tp)]*dz[rp][jp][tp]for rp in range (rr)) for jp in range (jj)) for tp in range (tt))for w in range (ww))for k in range (kk)))
    ...
    mdl.add_constraint(OTHR+setup+ openingw+ setupfirst<=ARCobj)
     # Constraints
  ...
  #production capacity constraint #1
    ...
    <=bigM *Z_P[(w,k,t)]  for w in range(ww)for k in range(kk)for t in range(tt))
