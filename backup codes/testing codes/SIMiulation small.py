...
#we do the simulation for small sized instances
...
for j in range (jj):
         CAP[j]=30000
         q_p[j][0]=sum(dz[r][j][0] for j in range (jj))*2
         q_p[j][1]=sum(dz[r][j][1] for j in range (jj))*1.5
         q_p[j][2]=sum(dz[r][j][2] for j in range (jj))*1.5
         q_p[j][3]=sum(dz[r][j][3] for j in range (jj))*2
         q_p[j][4]=sum(dz[r][j][4] for j in range (jj))*1.2
...
#setup
sigma=[[0,60*rr,80*rr,50],[40*rr,0,60*rr,50],[50*rr,40*rr,0,50],[50*rr,40*rr,0,50]]
#opening cost of warehouse
C_O_W=[0,120*rr,190*rr,120*rr]
...
zaman.close()
#%% simulation starts
benevis=open('SIMiulation small.txt','a')
benevis.write(str('***************'))
...
benevis=open('SIMiulation small.txt','a')
benevis.write(str('***************'))
benevis.write(str(jj)+'     '+str(kk)+'     '+str(tt)+'     '+str(rr)+'     '+str(ww)+'     '+str(zeta)+'  '+str(carpi)+'     '+str(dis_per_unit))
benevis.write(str('***************'))
dh=np.zeros((rr,jj,tt))
realized_demand=np.zeros((rr,jj,tt))
worsecase_demand=sum(sum(sum((D_W[rp][jp][tz]) for rp in range (rr)) for jp in range (jj)) for tz in range (tt))
